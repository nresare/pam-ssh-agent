...
```
VERSION=$(grep "^VERSION=" pam-ssh-agent-installer/setup.sh | cut -d'=' -f2)
makeself --gzip --sha256 ./pam-ssh-agent-installer ./pam-ssg-agent-installer-${VERSION}.sh "pam-ssh-agent ${VERSION}" ./setup.sh
```
